...
| 5 | Estado visual do passo | `pending` (círculo vazio) → `active` (círculo preenchido cor primária, texto bold) → `complete` (check verde quando todos os campos obrigatórios do passo estão preenchidos). Passos sem `requiredFields` (ex: Material Encaminhado, Cartuchos, Estojos) são automaticamente concluídos (`completed.add(stepId)`) — sem campos para validar = passo completo |
