...
| **ESLint** (`npm run lint`) | 🟡 OK com warnings | 0 err + 49 warn | 0 err | 📉 melhora |
...
> 📉 Em 01/07/2026 a limpeza incremental reduziu o lint de `214` para `49` warnings, mantendo `0` erros, `type-check`, testes e build passando.
...
| **01/07/2026** (limpeza incremental) | ✅ | 0 | 0 / 49 | 34/35 (+1 skip) | apontamentos remanescentes | Codex |
...
|---|---|---|
| `@typescript-eslint/no-explicit-any` | 22 | warning |
...
exportação, banco SQLite, services, logger, handlers IPC pequenos,
`template.handlers.ts` e remoção do skill local `grill-me` do repositório.
...
1. 🥇 **Tratar `laudo.handlers.ts`** em tranche curta, por concentrar `3` warnings restantes de `no-explicit-any`
2. 🥇 **Tratar `src/main/ipc/index.ts`** em corte separado, preservando contratos IPC e logs
